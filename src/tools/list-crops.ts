...
export function handleListCrops(db: Database, args: ListCropsArgs) {
  ...
  let sql = 'SELECT id, name, crop_group, typical_yield_t_ha, altitude_zone FROM crops WHERE jurisdiction = ?';
  ...
  const crops = db.all<{
    id: string; name: string; crop_group: string; typical_yield_t_ha: number; altitude_zone: string;
  }>(sql, params);
